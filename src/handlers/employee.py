...
def setup_employee_router(
    ...
    async def report_send(message: Message, state: FSMContext):
        ...
        if not task_id:
            await state.clear()
            await message.answer(TASK_NOT_FOUND_TEXT, reply_markup=get_employee_menu_keyboard())
            return
        ...
        if task.employee_id != message.from_user.id:
            await state.clear()
            await message.answer(TASK_ACTION_NOT_ALLOWED_TEXT, reply_markup=get_employee_menu_keyboard())
            return
        ...
        await state.clear()
        await message.answer(REPORT_SENT_TEXT, reply_markup=get_employee_menu_keyboard())
