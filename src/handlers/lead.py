...
from keyboards.role_menus import get_employee_selection_keyboard
...
def setup_lead_router(auth_service: AuthService, tasks_service: TasksService, visits_service: VisitsService):
    ...
    async def lead_deadline_calendar(call: CallbackQuery, state: FSMContext):
        ...
        if action == "pick":
            ...
            employee_ids = await asyncio.to_thread(auth_service.get_employees_for_manager, lead_id)

            if not employee_ids:
                await state.clear()
                await call.message.answer(LEAD_CREATE_TASK_NO_EMPLOYEES_TEXT, reply_markup=get_lead_tasks_keyboard())
                return
            ...
            for emp_id in employee_ids:
                ...
                try:
                    ...
                    logger.warning("Не удалось получить данные сотрудника %s: %s", emp_id, exc)
    ...
    async def lead_task_employee_select(message: Message, state: FSMContext):
        ...
        try:
            ...
            logger.exception("Ошибка записи задачи в Google Sheets: %s", e)
            await state.clear()
            await message.answer(f"Ошибка при создании задачи: {e}", reply_markup=get_lead_tasks_keyboard())
            return
