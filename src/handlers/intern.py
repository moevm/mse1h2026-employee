...
def setup_intern_router(
    ...
    @router.message(F.text == Buttons.INTERN_BIND_MANAGER)
    async def bind_manager_start(message: Message, state: FSMContext):
        ...
        if not lead_ids:
            await message.answer(BIND_MANAGER_NO_LEADS_TEXT, reply_markup=get_intern_menu_keyboard())
            return
        ...
        if not lead_options:
            await message.answer(BIND_MANAGER_ALREADY_BOUND_TEXT, reply_markup=get_intern_menu_keyboard())
            return
    ...
    async def bind_manager_selected(message: Message, state: FSMContext):
        ...
        await state.clear()
        if request is None:
            await message.answer(BIND_MANAGER_REQUEST_ALREADY_EXISTS_TEXT, reply_markup=get_intern_menu_keyboard())
        else:
            await message.answer(BIND_MANAGER_REQUEST_SENT_TEXT, reply_markup=get_intern_menu_keyboard())


    @router.message(F.text == Buttons.INTERN_DAILY_REPORT)
